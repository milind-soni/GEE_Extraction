...
input_csv = "/Users/milindsoni/Documents/projects/rice/bamboo/2022_v3.csv"
output_csv = "bamboovector_full_with_weather.csv"
...
def combine_temp_files():
    ...
    combined_df = pd.concat([pd.read_csv(f) for f in temp_files])

    # Determine the correct column names for latitude and longitude
    lat_column = next(
        (
            col
            for col in combined_df.columns
            if col.lower() in ["lat", "latitude", "lattitude"]
        ),
        None,
    )
    lon_column = next(
    ...
    if lat_column is None or lon_column is None:
        logging.error(
            f"Could not find latitude or longitude columns. Available columns: {combined_df.columns}"
        )
        return None

    combined_df = combined_df.drop_duplicates(subset=[lat_column, lon_column])
    combined_df = combined_df.sort_values(lat_column)
    return combined_df
...
def continue_processing():
    ...
    with tqdm(total=len(df) - start_index, desc="Processing rows") as pbar:
        for i, result in enumerate(
            ...
            if (i + 1) % SAVE_INTERVAL == 0 or i == len(df) - 1:
                ...
                # Combine all temporary files after each save
                combined_df = combine_temp_files()
                if combined_df is not None:
                    combined_df.to_csv(output_csv, index=False)
                    logging.info(f"Combined results saved to {output_csv}")
                else:
                    logging.error(
                        "Failed to combine temporary files. Check the logs for details."
                    )
    ...
    logging.info("Processing complete. Final combination of all temporary files...")
